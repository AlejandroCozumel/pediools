...
export interface DippingResult {
  systolic: {
    percentage: number;
    category: 'Normal dipper' | 'Non-dipper' | 'Reverse dipper' | 'Extreme dipper';
    normal: boolean;
    color: string;
  };
  diastolic: {
...
export class AmbulatoryBPUtils {
   ...
   */
  static calculateDipping(daytimeBP: number, nighttimeBP: number): {
    percentage: number;
    category: 'Normal dipper' | 'Non-dipper' | 'Reverse dipper' | 'Extreme dipper';
    normal: boolean;
    color: string;
    ...
    let category: 'Normal dipper' | 'Non-dipper' | 'Reverse dipper' | 'Extreme dipper';
    let color: string;
    let normal: boolean;
  ...
  static getReferenceData(
    ...
    if (height && genderData.height) {
      ...
      return genderData.height[closestHeight];
    }

    // Fallback to age-based data
    if (genderData.age && genderData.age[age]) {
      return genderData.age[age];
    }
    ...
    return genderData.age[closestAge];
  }
  ...
  static analyzePeriod(
    ...
    const systolicPercentile = this.calculatePercentile(systolic, periodRef?.systolic);
    const diastolicPercentile = this.calculatePercentile(diastolic, periodRef?.diastolic);
  ...
  static classifyAmbulatoryBP(
    results: { day?: AmbulatoryBPResult; night?: AmbulatoryBPResult; '24h'?: AmbulatoryBPResult },
    age: number,
    t: any
  ): { category: string; description: string; color: string; bgColor: string } {

    // Check if any period shows hypertension
    const hasHypertension = Object.values(results).some(result =>
    ...
    if (age >= 13) {
      const hasAdultHypertension = Object.values(results).some(result => {
        ...
        return result.systolic.value >= systolicThreshold || result.diastolic.value >= diastolicThreshold;
      });

      if (hasAdultHypertension) {
        return {
          category: t("ambulatory.classifications.hypertension.category"),
          description: t("ambulatory.classifications.hypertension.adolescentDescription"),
          color: "text-red-700",
    ...
    if (maxPercentile >= 95) {
      return {
        category: t("ambulatory.classifications.hypertension.category"),
        description: t("ambulatory.classifications.hypertension.pediatricDescription"),
        color: "text-red-700",
        bgColor: "bg-red-50 border-red-200"
      };
    }
  ...
  static analyzeAmbulatoryBP(
    ...
    const results: { day?: AmbulatoryBPResult; night?: AmbulatoryBPResult; '24h'?: AmbulatoryBPResult } = {};
    ...
    if (measurements.systolicDay && measurements.diastolicDay &&
        measurements.systolicNight && measurements.diastolicNight) {
      dipping = {
        systolic: this.calculateDipping(measurements.systolicDay, measurements.systolicNight),
        diastolic: this.calculateDipping(measurements.diastolicDay, measurements.diastolicNight)
      };
    }

    // Classify overall result
    const classification = this.classifyAmbulatoryBP(results, patientInfo.age, t);
  ...
  static generateBedsideCard(
    ...
    const dayPercentiles = {
      systolic: this.calculatePercentile(measurements.systolicDay, referenceData.day?.systolic),
      diastolic: this.calculatePercentile(measurements.diastolicDay, referenceData.day?.diastolic)
    };

    const nightPercentiles = {
      systolic: this.calculatePercentile(measurements.systolicNight, referenceData.night?.systolic),
      diastolic: this.calculatePercentile(measurements.diastolicNight, referenceData.night?.diastolic)
    };
    ...
    const referenceTable = {
      day: {
        p5: `${referenceData.day?.systolic.p5 || '--'}/${referenceData.day?.diastolic.p5 || '--'}`,
        p50: `${referenceData.day?.systolic.p50 || '--'}/${referenceData.day?.diastolic.p50 || '--'}`,
        p90: `${referenceData.day?.systolic.p90 || '--'}/${referenceData.day?.diastolic.p90 || '--'}`,
        p95: `${referenceData.day?.systolic.p95 || '--'}/${referenceData.day?.diastolic.p95 || '--'}`,
        p99: `${referenceData.day?.systolic.p99 || '--'}/${referenceData.day?.diastolic.p99 || '--'}`
      },
      night: {
        p5: `${referenceData.night?.systolic.p5 || '--'}/${referenceData.night?.diastolic.p5 || '--'}`,
        p50: `${referenceData.night?.systolic.p50 || '--'}/${referenceData.night?.diastolic.p50 || '--'}`,
        p90: `${referenceData.night?.systolic.p90 || '--'}/${referenceData.night?.diastolic.p90 || '--'}`,
        p95: `${referenceData.night?.systolic.p95 || '--'}/${referenceData.night?.diastolic.p95 || '--'}`,
        p99: `${referenceData.night?.systolic.p99 || '--'}/${referenceData.night?.diastolic.p99 || '--'}`
      }
    };
