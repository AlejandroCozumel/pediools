...
const WeeklySchedule: React.FC<WeeklyScheduleProps> = ({
  ...
  const criticalErrors = useMemo(() =>
    validationErrors.filter(error => error.type === 'error' && !error.dayOfWeek),
  [validationErrors]);

  // Filter only warnings to display at the top
  const generalWarnings = useMemo(() =>
    validationErrors.filter(error => error.type === 'warning' && !error.dayOfWeek),
  [validationErrors]);
  ...
  }, [weeklySchedule, daysOfOperation, onWeeklyScheduleChange, onDaysOfOperationChange]);
  ...
  const handleBreakAdded = useCallback((
    ...
    const updatedSchedule = [...weeklySchedule];
    const dayBreaks = [...updatedSchedule[dayIndex].breaks, breakPeriod];
  ...
  const handleDefaultSlotDurationChange = useCallback((value: string) => {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-medical-600">
        Set your regular weekly availability schedule. This will be used to generate appointment slots for patients to book.
      </p>
      ...
      <div className="max-w-xs">
        ...
        >
          <SelectTrigger className="border-medical-200">
            <SelectValue>
              {slotDurationOptions.find(option => option.value === defaultSlotDuration)?.label}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-4">
        {weeklySchedule.map((day, index) => (
          <DayScheduleComponent
            ...
            errors={validationErrors.filter(error => error.dayOfWeek === day.dayOfWeek)}
