...
const { chartJsData, patientDataPointsWithPercentile } = useMemo(() => {
  ...
  const externalTooltipHandler = useCallback(
    (context: { chart: ChartJS; tooltip: TooltipModel<"line"> }) => {
      ...
      const gestationalWeeks = chart.scales.x.getValueForPixel(tooltip.caretX);
      if (gestationalWeeks === undefined || gestationalWeeks === null) {
        setTooltipState((prev) => ({ ...prev, visible: false }));
        return;
      }

      const title = `Gestational Age: ${formatGestationalAge(gestationalWeeks)}`;
      ...
      if (firstDataset?.data) {
        (firstDataset.data as { x: number }[]).forEach((point, index) => {
          if (point && typeof point.x === "number") {
            const diff = Math.abs(point.x - gestationalWeeks);
            if (diff < minDiff) {
              minDiff = diff;
              dataIndex = index;
            }
      ...
      const patientPointAtHover = patientDataPointsWithPercentile.find(
        (p) => Math.abs(p.x - gestationalWeeks) < weeksTolerance
      );
