...
import cdcHeightData from "@/app/data/who-data-height.json";
...
const WHOChartHeight = ({ data }: ChartProps) => {
  ...
  function findClosestDataPoint(
    ...
    const filteredData = cdcHeightData.filter((point) => point.Sex === sex);
    ...
    const lowerPoint = point1.Agemos < point2.Agemos ? point1 : point2;
    const upperPoint = point1.Agemos > point2.Agemos ? point1 : point2;
  ...
  const generateFullCurveData = (
    ...
    // Create a more dense set of ages from 1 to 24 months
  ...
  const yAxisTicks = Array.from({ length: 15 }, (_, i) => i * 5 + 35); // 35 to 105 by 5s
  const yAxisTicksMobile = Array.from({ length: 8 }, (_, i) => i * 10 + 35); // 35 to 105 by 10s
