...
export function AmbulatoryReferenceCard() {
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('ambulatory-bp-reference-card');
    if (printContent) {
      const newWindow = window.open('', '_blank', 'height=600,width=800');
      newWindow?.document.write('<html><head><title>Print Reference Card</title>');
      const styles = Array.from(document.styleSheets)
        .map(styleSheet => styleSheet.href ? `<link rel="stylesheet" href="${styleSheet.href}">` : '')
        .join('');
      newWindow?.document.write(styles);
      newWindow?.document.write('<style>@media print { body { -webkit-print-color-adjust: exact; } .print-hidden { display: none !important; } }</style>');
