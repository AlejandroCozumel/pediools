...
type BSANormalizedFormValues = z.infer<typeof bsaNormalizedSchema>;
...
const dosageTypeOptions = [
  { value: "/1.73m²/day", label: "/1.73m²/day", description: "Per 1.73m² per day (normalized)" },
  { value: "/1.73m²/dose", label: "/1.73m²/dose", description: "Per 1.73m² per dose (normalized)" },
...
// Standard adult BSA for normalization
const STANDARD_BSA = 1.73;

export function BSANormalizedDoseForm() {
  const form = useForm<BSANormalizedFormValues>({
    resolver: zodResolver(bsaNormalizedSchema),
    defaultValues: {
      dose: "",
      doseUnit: "mg",
      dosageType: "/1.73m²/day",
      frequency: "qD",
      weight: "",
      weightUnit: "kg",
      height: "",
  ...
  // Calculate results with BSA-normalized formulas
  ...
  const actualBSA = (weight > 0 && height > 0) ? calculateBSA(weightInKg, heightInCm) : 0;

  // Calculate BSA ratio for normalization
  const bsaRatio = actualBSA / STANDARD_BSA;
  ...
  // Calculate dosages based on normalized BSA
  ...
  if (dosageType === "/1.73m²/day") {
    // Dose is normalized to 1.73m², adjust for actual BSA
    dailyDosageInMg = dose * bsaRatio;
    perDosageInMg = dailyDosageInMg / (freqMap[frequency] || 1);
  } else if (dosageType === "/1.73m²/dose") {
    // Dose per administration, normalized
    perDosageInMg = dose * bsaRatio;
  ...
  if (doseUnit === "ml" && dosageType.includes("/1.73m²")) {
    perDoseVolume = dosageType === "/1.73m²/day"
      ? (dose * bsaRatio) / (freqMap[frequency] || 1)
      : dose * bsaRatio;
  } else if (doseUnit === "tablet" && dosageType.includes("/1.73m²")) {
    perDoseVolume = dosageType === "/1.73m²/day"
      ? (dose * bsaRatio) / (freqMap[frequency] || 1)
      : dose * bsaRatio;
    tabletsPerDose = perDoseVolume;
    tabletsPerDay = dose * bsaRatio * (dosageType === "/1.73m²/day" ? 1 : freqMap[frequency] || 1);
  }
          ...
          <Card className="bg-medical-50/20 border-medical-100/50">
            <CardHeader className="pb-4">
              <CardTitle className="text-lg flex items-center gap-2 text-medical-900">
                ...
                BSA-Normalized Dosage Prescribed
              </CardTitle>
            </CardHeader>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col justify-between gap-2">
                      <FormLabel className="flex items-center gap-2">
                        Dose Amount
                        <InfoTooltip content="The amount of drug normalized to standard BSA of 1.73 m²">
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-medical-900">Actual BSA:</span>
                        <Badge variant="outline" className="border-medical-300 text-medical-800">
                          {actualBSA.toFixed(3)} m²
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-medical-900">BSA Ratio (vs 1.73m²):</span>
                        <Badge variant="outline" className="border-medical-300 text-medical-800">
                          {bsaRatio.toFixed(3)}
                        </Badge>
                      </div>
                ...
                <Alert className="bg-medical-50/50 border-medical-200/50">
                  <AlertDescription>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline" className="border-medical-300 text-medical-800">
                        {dose} {doseUnit}{dosageType} • Ratio: {bsaRatio.toFixed(3)}
                      </Badge>
                    </div>
                  </AlertDescription>
                </Alert>
            ...
            <Card className="bg-medical-50/20 border-medical-100/50">
              <CardHeader className="pb-4">
                <CardTitle className="text-lg flex items-center gap-2 text-medical-900">
                  ...
                  Calculated BSA-Normalized Dosage
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6 !pt-0">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">
                      Per Dose Volume
                      <InfoTooltip content="Calculated volume per dose based on normalized BSA">
                        <Info className="h-4 w-4 text-medical-500 cursor-pointer" />
                      </InfoTooltip>
                    </FormLabel>
                    ...
                    <AlertDescription>
                      <div className="space-y-4">
                        <div className="font-semibold text-medical-900">
                          Calculated BSA-Normalized Dosages:
                        </div>
