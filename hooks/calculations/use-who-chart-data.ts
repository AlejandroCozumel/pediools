// hooks/calculations/use-who-chart-data.ts
...
function validateGrowthData(data: GrowthData): string | null {
  ...
  for (const measurement of data.measurements) {
    ...
    if (ageInMonths < 0 || ageInMonths > 24) {
      return "Age must be between 0 and 24 months";
    }
...
const fetchWHOChartDataClient = async (
  searchParams: URLSearchParams
): Promise<WHOChartResult> => {
  ...
  const weightData: GrowthData = JSON.parse(decodeURIComponent(encodedWeightData));
  const heightData: GrowthData = JSON.parse(decodeURIComponent(encodedHeightData));
  ...
  const weightPercentileResults = calculateGrowthPercentiles(
    weightData,
    whoWeightData as CDCDataPoint[]
  );
  const heightPercentileResults = calculateGrowthPercentiles(
    heightData,
    whoHeightData as CDCDataPoint[]
  );

  // For client-side, we don't fetch patient data from database
...
export const useWHOChartData = (searchParams: URLSearchParams) => {
  return useQuery({
