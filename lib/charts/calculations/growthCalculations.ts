...
export function calculateGrowthChartData(
  ...
  if (patientMeasurements.length > 0) {
    progressionData = patientMeasurements
      ...
      .filter((measurement) => {
        ...
        if (calculationId) {
          const currentCalculation = patientMeasurements.find(
  ...
  return {
    success: true,
    calculationId: patientMeasurements[patientMeasurements.length - 1]?.id,
    data: {
    ...
    progressionData: progressionData,
    patientDetails: null, // Will be handled separately if needed
  };
}